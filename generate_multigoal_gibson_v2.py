...
if __name__ == "__main__":
    ...
    saved_dir = './image-goal-nav-dataset/gibson/{}goal'.format(num_goals)
    if os.path.exists(saved_dir): shutil.rmtree(saved_dir)
    os.mkdir(saved_dir)

    info_dir = './image-goal-nav-dataset/gibson/{}goal_info'.format(num_goals)
    ...
    num_sampling = 100
    tolerance = 600
    # NOTE: if obstacle_dist > 0.7, no episode will be found for Swormville, so we set 0.4 here.
    dist_upper, dist_lower, obstacle_dist, proximity = 10, 1.0, 0.4, 1.5
    ...
    def validate(path, pathfinder, next_goal, prev_goal_list):
        is_valid = False
        path.requested_start = prev_goal_list[-1]
        path.requested_end = next_goal
        found_path = pathfinder.find_path(path) # bool

        geodesic_dist = path.geodesic_distance
        path_points = path.points

        # print(sim.pathfinder.is_navigable(next_goal))
        # print(sim.pathfinder.distance_to_closest_obstacle(next_goal, max_search_radius), obstacle_dist)
        # print(dist_upper, geodesic_dist, dist_lower)
        # print(next_goal[1], goal_list[-1][1])
        # input()
        if  sim.pathfinder.is_navigable(next_goal) \
            and sim.pathfinder.distance_to_closest_obstacle(next_goal, max_search_radius) >= obstacle_dist \
            and geodesic_dist != float("inf") \
            and (dist_upper > geodesic_dist > dist_lower) \
            and (next_goal[1] - goal_list[-1][1]) < 1e-6:
            is_valid = True
    ...
    for i in range(len(scene_name_list)):
        ...
        #if scene_name != "Swormville": continue
        ...
        sim_settings = {
            ...
            "seed": i,  # used in the random navigation
        ...
        # print(scene_name, sim.pathfinder.get_bounds(), sim.pathfinder.navigable_area); input()
            ...
            for j in range(num_episodes):    
                while True:
                    ...
                    for goal_idx in range(num_goals):
                        ...
                        if goal_idx != num_goals - 1:
                            ...
                            while cnt < num_sampling and n <= tolerance:
                                ...
                                if not validate(path, sim.pathfinder, next_goal, goal_list) \
                                ...
                                # select the point with the largest distance away from the last goal
                    ...
                    if success:
                        ...
                        print("[{}] Cannot find a goal. Reselect the starting point.".format(scene_name))
    ...
    s = "{} scenes, {} trajectories in total. Avg/min/max geodesic distance: {:.2f} / {:.2f} / {:.2f}. Time elapsed: {:.2f}s\n".format(
        len(scene_name_list), len(geodesic_dist_lst), avg_total_geodesic_dist, min(geodesic_dist_lst), max(geodesic_dist_lst), time.time()-t
        )
    print(s)
    dataset_info_txt.write(s)
